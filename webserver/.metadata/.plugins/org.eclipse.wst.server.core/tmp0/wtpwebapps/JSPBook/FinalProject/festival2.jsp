...
<%		}
		if(chk.contains(name)){
			%>
			<input type="checkbox" name="festival" value="<%=name %>,<%=latitude %>,<%=longitude%>,<%=web %>,<%=contents %>" checked><%=name %><br>
			<p id="tw">tel : <%=tel %><br>주소 : <%=address %></p><br>
<%		}else{%>
			<input type="checkbox" name="festival" value="<%=name %>,<%=latitude %>,<%=longitude%>,<%=web%>,<%=contents%>"><%=name %><br>
			<p id="tw">tel : <%=tel %><br>주소 : <%=address %></p><br>
<%}
}
	pre=area2;
...
<input class="btn" type="submit" value="관광지 선택하러 가기 " onclick="javascript:form.action='spot1.jsp';">
</div>
</form>
